...
class answerCandidate:
    ...
    def calcScore(self, qtList, countCharDict, debug=False, includingObj = [], use_elmo=False):
        ...
        if use_elmo and len(pre) != 0:
            ...
            q_embeddings = torch.tensor(embeddings[0].mean(1)).to(DEVICE).unsqueeze(0)
            p_embeddings = torch.tensor(embeddings[1].mean(1)).to(DEVICE).unsqueeze(0)
            q_rep = model(q_embeddings)
            p_rep = model(p_embeddings)

            # code.interact(local=locals())
            scorePre = (q_rep * p_rep).sum(-1, keepdim=True).item()
