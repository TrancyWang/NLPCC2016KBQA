...
class answerCandidate:
    ...
    def calcScore(self, qtList, countCharDict, debug=False, includingObj = [], use_elmo=False):
        ...
        if use_elmo and len(pre) != 0:
            ...
            embeddings = e.sents2elmo(data)
            pre_embeddings = np.mean(embeddings[0], 0)
            qWithoutSub_embeddings = np.mean(embeddings[1], 0)
            scorePre = 1 - cosine(pre_embeddings, qWithoutSub_embeddings)
